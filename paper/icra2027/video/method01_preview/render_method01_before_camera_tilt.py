...
@lru_cache(None)
def cameras():
 sprite,_=camera_asset();size=(round(139*.62*S),round(108*.62*S))
...
def draw_observations(c,D,t,sf):
 ...
 _,lens=camera_asset()
 upper_lens=np.array([75.,383.])+np.array([lens[0],108-lens[1]])*.62
 lower_lens=np.array([205.,383.])+lens*.62
