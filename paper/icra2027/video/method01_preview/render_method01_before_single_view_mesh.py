...
def data():
 ...
 return dict(stereo_crops=stereo_crops,surface_ids=surface_ids,surface_proj=surface_proj,hp=hp,display_pos=display_pos,kin=kin,tr=tr,force=force,camera=camera,camera_right=camera_right,ids=ids,hsel=hsel,hproj=hproj,sproj=sproj,crop=crop,rgb=rgb,reference=reference,disp_max_m=disp_max_m)
...
def static_frame():
 ...
 for x,title,sub in [(455,'Track texture','Two calibrated views'),(650,'Triangulate','3D surface points'),(843,'Extend motion inside','Square-symmetric field')]:
...
def draw_method01(t):
 ...
 # Simultaneous calibrated camera views and their real shared correspondences.
 sf=int(np.clip(round((.10+1.80*phase)*100),0,200));tr=D['tr'];colors=['#ffb321','#00d8ed','#ff65d4']
 endpoints=[];histories=[]
 for ci,y in [(0,484),(1,566)]:
  crop=D['stereo_crops'][ci];src=D['camera'] if ci==0 else D['camera_right']
  c.paste(Image.fromarray(src[sf]).crop(tuple(crop)).convert('RGB'),(369,y,172,56))
  c.text(455,y-22,'Left view' if ci==0 else 'Right view',15.5,BLUE,True,'mt')
  scale=np.array([172/(crop[2]-crop[0]),56/(crop[3]-crop[1])])
  view=[];trails=[]
  for fid in D['ids']:
   trail=(tr['pixels'][ci,10:sf+1,fid]-crop[:2])*scale+[369,y]
   view.append(trail[-1]);trails.append(trail)
  endpoints.append(view);histories.append(trails)
 proj=D['surface_proj'];surface=tr['world'][sf,D['surface_ids']];cloud=proj(surface)
 selected=proj(tr['world'][sf,D['ids']])
 # Correspondence links are a schematic graphic, with endpoints driven by real
 # image pixels and their triangulated 3D positions, not a fake camera baseline.
 for ci in [0,1]:
  for q,dest,col in zip(endpoints[ci],selected,colors):
   color=tuple(int(col[j:j+2],16) for j in [1,3,5])
   muted=tuple(np.rint(np.array(color)*.64+np.array([244,246,247])*.36).astype(int))
   c.line([q,dest],muted,1.15)
 # Depth-shaded surface samples: never fill an unobserved interior or rear face.
 dep=surface@proj.view;alpha=(dep-dep.min())/max(np.ptp(dep),1e-12)
 for idx in np.argsort(dep):
  color=tuple(np.rint(np.array([169,194,207])*(1-alpha[idx])+np.array([78,133,164])*alpha[idx]).astype(int))
  c.dot(cloud[idx],1.4,color)
 # Mark the same three material features in both images and in 3D.
 for ci in [0,1]:
  for trail,col in zip(histories[ci],colors):
   if len(trail)>1:c.line(trail,'#17323d',3.7);c.line(trail,col,1.8)
   c.dot(trail[0],2.4,'#17323d','white')
   q=trail[-1]
   for sx in [-1,1]:
    for sy in [-1,1]:
     corner=q+[sx*6,sy*6];pts=[corner-[sx*3.7,0],corner,corner-[0,sy*3.7]]
     c.line(pts,'#17323d',3.8);c.line(pts,col,2)
   c.dot(q,1.8,col,'#17323d')
 for fid,q,col in zip(D['ids'],selected,colors):
  history=proj(tr['world'][10:sf+1,fid])
  if len(history)>1:c.line(history,'#f4f6f7',3.6);c.line(history,col,1.8)
  c.dot(q,4,col,INK);c.dot(q,1.2,'white')
 # Fixed coordinate triad identifies the oblique 3D view without rotating it.
 origin=np.array([575.,612.])
 for axis,label in enumerate(['x','y','z']):
  v=np.array([proj.right[axis],-proj.up[axis]])*18
  c.arrow(origin,origin+v,'#648697',1,4)
  dest=origin+v*1.3;c.text(dest[0],dest[1],label,11.5,MUTED,anchor='mm')
