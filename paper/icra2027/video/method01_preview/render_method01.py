...
P=Path(__file__).resolve().parent;ROOT=P.parents[3];DURATION=32;FPS=25;S=2
...
def draw_observations(c,D,t,sf,clock=0.):
...
def static_frame():
 ...
 # The last equation is the trajectory ODE for particle p, evaluated in the
 # inferred Eulerian velocity field, not an additional identification equation.
 for x,title,sub in [(455,'Surface reconstruction','Axisymmetry'),(650,'Same volume','∇ · v = 0'),(843,'Particle advection','ẋₚ = v(xₚ, t)')]:
 ...
 for j in range(100):c.line([(1074+j,383),(1074+j,389)],tuple((np.array(DISPLACEMENT(j/99)[:3])*255).astype(int)),1)
 c.text(1124,394,'Displacement (mm)',13,MUTED,anchor='mt')
 c.text(1065,386,'0',12.5,MUTED,anchor='rm');c.text(1183,386,f"{data()['disp_max_m']*1000:g}",12.5,MUTED,anchor='lm')
 ...
 for val in [0,10]:
  y=y0+h-val/12*h;c.line([(x0,y),(x0+w,y)],'#dce4e8',.6);c.text(x0-8,y,str(val),13,MUTED,anchor='rm')
 for tv in [0,1,2]:c.text(x0+tv/2*w,y0+h+7,str(tv),13,MUTED,anchor='mt')
 c.text(1027,499,'N',13,MUTED,anchor='mt');c.text(1228,644,'s',13,MUTED)
 return im
...
def draw_method01(t,reveal=True):
 ...
 xx=1039+source_t/2*178;c.line([(xx,503),(xx,637)],'#e5c5ae',.7)
 # Translate the whole reconstruction stage without rescaling its contents.
 bounds=(350*S,137*S,951*S,693*S);middle=im.crop(bounds)
...
def main():
 ...
 'alignment':'Top diagrams and output motion centered at y=284.5. Lower method row and taller input image centered at y=550; force X-axis aligned to lower captions at y=637. Three top method diagrams have identical projection scales. All three stages use equal-height rounded outlines and a common background; Camera centers, stage dividers and both bracket arrows align at y=414, balancing visible image gaps in the initial frame (lower plate begins at y=461, after 8 px of empty image background). Bracket spans y=137..691 symmetrically, matching the block outline top and bottom. Stereo display boxes grow vertically while retaining fixed aspect-correct projections and center y=550; force X-axis stays y=637. Reconstruction stage translated right by 17 px without scaling; equal 41 px gaps separate the stages. Identical brackets at x=343 and x=983 terminate at borders x=367 and x=1007. No arrow passes through the cameras. Small inputs-panel annotations and force-axis labels are enlarged.'},indent=2)+'\n')
