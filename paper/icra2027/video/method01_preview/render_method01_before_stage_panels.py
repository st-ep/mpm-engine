...
def data():
 ...
 sproj=Projection(local.reshape(-1,3),(774,517,153,108),az=-52,el=24)
 context_proj=Projection(X[10:191:10].reshape(-1,3),(884,488,44,30),az=-52,el=24)
...
def draw_stereo_interior(c,D,frame):
 ...
 def boundary(grid):return np.vstack([grid[0],grid[1:,-1],grid[-1,-2::-1],grid[-2:0:-1,0]])
 # Small whole-specimen outline locates the magnified material region.
 cp=D['context_proj']
 for axis in range(3):
  for i in [0,10]:
   for j in [0,10]:
    sl=[i,j];sl.insert(axis,slice(None))
    c.line(cp(lattice[tuple(sl)]),'#c0cfd7',.65)
 region=cp(boundary(local[:,0,:]));c.poly(region,'#dae9f0')
 c.line(np.vstack([region,region[0]]),'#739cb2',.8)
 c.line([region[0],[780,534]],'#ccdbe3',.65)
 c.line([region[-1],[780,611]],'#ccdbe3',.65)
...
def static_frame():
 ...
 c.text(44,22,'FORM · From Observed Response to Material laws',20,TEAL,True)
 ...
 c.text(1124,477,'Measured normal force',18,INK,True,'mt')
