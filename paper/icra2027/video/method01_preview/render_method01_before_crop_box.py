...
def data():
 ...
 hproj=[Projection(hbound,b) for b in [(370,213.5,171,142),(564.5,213.5,171,142),(757.5,213.5,171,142),(1007,273,219,157)]]
...
def draw_observations(c,D,t,sf):
 ...
 for src,box in [(Image.fromarray(rgb),(44,201,270,167)),(Image.fromarray(D['camera'][sf]).convert('RGB').crop((170,400,1160,964)),(44,473,270,154))]:
 ...
 for target in targets:
  dist=np.linalg.norm(pix[172]-target,axis=1);dist[~valid]=np.inf;fid=int(np.argmin(dist));ends.append(target+pix[sf,fid]-pix[172,fid])
 ends=(np.array(ends)-[170,400])*[270/990,154/564]+[44,473]
 ...
 mask[:4]=0;mask[122:]=0;mask[:,:30]=0;mask[:,205:]=0
 ...
 slope=-(ends[1,1]-ends[0,1])/(ends[1,0]-ends[0,0]);xs=np.linspace(xx.min(),xx.max(),700);ys=center[1]+slope*(270/235)/(167/145)*(xs-center[0]);ix=np.clip(np.rint(xs).astype(int),0,234);iy=np.clip(np.rint(ys).astype(int),0,144);inside=mask[iy,ix];which=np.flatnonzero(inside);endpoints=np.c_[xs[which[[0,-1]]],ys[which[[0,-1]]]]*[270/235,167/145]+[44,201]
 ...
 for side in [0,1]:
  ...
  curve=np.array(curve)*[270/235,167/145]+[44,201];c.line(curve,'#123a38',2.5);c.line(curve,'#77ead2',.8)
...
def static_frame():
 ...
 c.d.rectangle((44*S,198*S,341*S,630*S),fill=BG)
 ...
 c.line([(968,284.5),(986,284.5),(986,449),(1001,449)],TEAL,1.5)
 c.line([(968,550),(986,550),(986,449)],TEAL,1.5);c.dot((986,449),2.4);c.arrow((989,449),(1006,449),TEAL,1.5)
 ...
 for j in range(80):c.line([(1083+j,469),(1083+j,474)],tuple((np.array(DISPLACEMENT(j/79)[:3])*255).astype(int)),1)
 c.text(1124,492,'Displacement (mm)',12,MUTED,anchor='mt')
 c.text(1083,478,'0',10.5,MUTED,anchor='mt');c.text(1163,478,f"{data()['disp_max_m']*1000:g}",10.5,MUTED,anchor='mt')
...
def draw_method01(t):
 ...
 for fid in pids:
  ...
  for j in range(len(trail)-1):
   a=(j+1)/max(1,len(trail)-1);col=tuple(np.rint(np.array([244,246,247])*(1-.85*a)+np.array([21,94,103])*.85*a).astype(int))
   c.line(trail[j:j+2],col,1.45)
