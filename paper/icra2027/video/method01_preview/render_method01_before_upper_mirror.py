...
def data():
 ...
 # Three valid lower-surface features keep the close-up entirely on texture.
 ids=[828,814,361]
 ...
 return dict(hp=hp,display_pos=display_pos,kin=kin,tr=tr,force=force,camera=camera,camera_right=camera_right,ids=ids,hsel=hsel,hproj=hproj,sproj=sproj,crop=crop,rgb=rgb,reference=reference,disp_max_m=disp_max_m)
...
@lru_cache(None)
def cameras():
 # Both lenses face their observed specimen: upper right / lower left.
 ...
 for upper,cx in [(True,118.),(False,248.)]:
  icon=source.transpose(Image.Transpose.FLIP_LEFT_RIGHT) if upper else source
  point=lens*S
  if upper:point=np.array([source.width-point[0],point[1]])
  w,h=icon.size;M=cv2.getRotationMatrix2D((w/2,h/2),25,1.)
...
def static_frame():
 ...
 for x,title,sub in [(455,'Track texture','Texture close-up'),(650,'Triangulate','Calibrated stereo views'),(843,'Extend motion inside','Square-symmetric field')]:
 ...
 c.text(655,701,'Sequences slowed · hardware views synchronized · RGB-D diagrams centered laterally for display',12,MUTED,anchor='mt')
 return im
...
def draw_method01(t):
 ...
 # Magnified texture-only view, using three saved valid feature identities.
 sf=int(np.clip(round((.10+1.80*phase)*100),0,200));tr=D['tr'];crop=D['crop'];raw=D['camera'][sf]
 pic=Image.fromarray(raw).crop(tuple(crop)).convert('RGB');c.paste(pic,(369,490,172,113))
 origin2=np.array([369.,490.]);sc=np.array([172/(crop[2]-crop[0]),113/(crop[3]-crop[1])]);colors=['#ffb321','#00d8ed','#ff65d4']
 for fid,col in zip(D['ids'],colors):
  history=tr['pixels'][0,10:sf+1,fid];q=(history-crop[:2])*sc+origin2
  if len(q)>1:c.line(q,'#17323d',4.5);c.line(q,col,2.3)
  c.dot(q[0],3.4,'#17323d','white');c.dot(q[0],1.6,col)
  end=q[-1]
  # Four bright patch corners leave the tracked texture itself visible.
  for sx in [-1,1]:
   for sy in [-1,1]:
    corner=end+[sx*8,sy*8]
    pts=[corner-[sx*5,0],corner,corner-[0,sy*5]]
    c.line(pts,'#17323d',4.5);c.line(pts,col,2.4)
  c.dot(end,2.2,col,'#17323d')
 # Two actual cropped camera views show the same correspondence in gold.
 # Their arrangement is an explanatory diagram, not a camera-calibration drawing.
 fid=D['ids'][0];traj=tr['world'][10:191,fid];tp=Projection(traj,(641,568,20,19),az=-55,el=25)
 point=tp(tr['world'][sf,[fid]])[0];av=[]
 for ci,x in [(0,578),(1,660)]:
  pix=tr['pixels'][ci,10:191,fid];center=(pix.min(0)+pix.max(0))/2
  h=max(float(np.ptp(pix[:,1]))+36,75);w=h*64/55
  box=np.rint([center[0]-w/2,center[1]-h/2,center[0]+w/2,center[1]+h/2]).astype(int)
  src=D['camera'] if ci==0 else D['camera_right']
  c.paste(Image.fromarray(src[sf]).crop(tuple(box)).convert('RGB'),(x,504,64,55))
  c.line([(x,504),(x+64,504),(x+64,559),(x,559),(x,504)],'#99b1bf',.8)
  c.text(x+32,488,'Left view' if ci==0 else 'Right view',15.5,BLUE,True,anchor='mt')
  q=(tr['pixels'][ci,sf,fid]-box[:2])*[64/(box[2]-box[0]),55/(box[3]-box[1])]+[x,504];av.append(q)
 # Distinct viewing rays converge at a single moving triangulated point.
 for q in av:
  c.line([q,point],'#f4f6f7',4.5);c.arrow(q,point,BLUE,2.2,6)
 for q in av:
  c.dot(q,4.0,'#ffb321','#17323d');c.dot(q,1.3,'white')
 c.dot(point,5.0,'#ffb321','#17323d');c.dot(point,2,'white')
 c.text(651,593,'3D point',12,BLUE,True,'mt')
