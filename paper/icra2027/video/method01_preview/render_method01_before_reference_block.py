...
def data():
 ...
 # Fixed world origin and scale through the initial camera turn: no auto-fit.
 bound=tr['world'][10:191:5][:,surface_ids].reshape(-1,3);surface_origin=bound.mean(0)
 extent=np.zeros(2)
 for az in np.linspace(-78,-43,20):
  right,up,_=basis(az,24);relative=bound-surface_origin
  extent=np.maximum(extent,np.max(abs(np.c_[relative@right,-relative@up]),axis=0))
 surface_scale=min(150/(2*extent[0]),126/(2*extent[1]))


 reference,_,_=lerp_frame(hp['pos'],hp['times'],.05)
 displacement=np.linalg.norm(hp['pos'][hsel]-reference,axis=2)
 disp_max_m=float(np.ceil(displacement.max()/.005)*.005)
 return dict(texture_crop=texture_crop,surface_triangles=surface_triangles,surface_origin=surface_origin,surface_scale=surface_scale,stereo_crops=stereo_crops,surface_ids=surface_ids,surface_proj=surface_proj,hp=hp,display_pos=display_pos,kin=kin,tr=tr,force=force,camera=camera,camera_right=camera_right,ids=ids,hsel=hsel,hproj=hproj,sproj=sproj,crop=crop,rgb=rgb,reference=reference,disp_max_m=disp_max_m)

def surface_view(D,t):
 # Ease through a small introductory orbit, then remain fixed for deformation.
 a=float(np.clip(t/2.5,0,1));a=a*a*(3-2*a)
 proj=object.__new__(Projection);proj.right,proj.up,proj.view=basis(-78+35*a,24)
 proj.origin=D['surface_origin'];proj.mid=np.zeros(2);proj.center=np.array([654.,555.]);proj.scale=D['surface_scale']
 return proj
...
def draw_method01(t):
 ...
 for idx in order:
  ...
  color=tuple(np.rint(np.array([239,244,246])*(1-shade)+np.array([216,232,240])*shade).astype(int))
  c.poly(cloud[tri],color)
  for j in range(3):edges.add(tuple(sorted((int(tri[j]),int(tri[(j+1)%3])))))
 for a,b in edges:c.line(cloud[[a,b]],'#b6cdd9',.55)
 for idx in np.argsort(dep):
  color=tuple(np.rint(np.array([149,178,195])*(1-alpha[idx])+np.array([66,121,154])*alpha[idx]).astype(int))
  c.dot(cloud[idx],1.35,color)
