...
def hollow_shell(cx):
 ...
 patch=im.crop(box);patch=Image.blend(patch,Image.new('RGB',patch.size,'white'),.76)
...
# A clean constraint bridge replaces the rejected overlaid cutaway entirely.
# Incompressibility is only one constraint; the boundary/contact information
# and the minimum-dissipation selection prior are also stated explicitly.
text(298,16,'Boundary motion',13,INK,True)
arrow((298,35),(298,46),'#85aaa3',1.1,3.5)
text(298,51,'∇ · v = 0',25,TEAL)
text(298,82,'Preserve volume',11.5,MUTED)
line([(226,103),(370,103)],'#dce8e3',.7)
text(298,113,'Match surface + contact motion',10.5,MUTED)
...
ids.extend(chosen)
text(91,113,'Surface only',10.5,MUTED)
text(508,113,'Reconstructed particles',10.5,MUTED)
for cx,title,sub in [(91,'Reconstruct the shape','Silhouettes + axisymmetry'),(298,'Reconstruct the flow','Minimum-dissipation prior'),(508,'Move the particles','Positions → motion + deformation')]:
...
(P/'rgbd_shape_flow_particles_provenance.json').write_text(json.dumps({
 ...
 'shell':'Same display-only radial envelope of supplied ep0001 reconstructed particles. Lightened thin-film rendering with closed top-cap grid and empty interior in first panel. Hollow appearance communicates missing interior kinematics, not a physically hollow specimen.',
 'middle':'Constraint diagram. Divergence-free motion with moving-surface/contact constraints and minimum-dissipation prior. No synthetic flow arrows or overlaid body.',
