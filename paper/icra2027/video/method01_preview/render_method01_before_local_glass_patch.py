...
def data():
 ...
 # Even coverage of the actually observed surface, with persistent feature IDs.
 valid_ids=np.flatnonzero(tr['valid'][10:191].all(0));ref=tr['world'][10,valid_ids]
 xz=ref[:,[0,2]];lo=xz.min(0);hi=xz.max(0)
 gx,gz=np.meshgrid(np.linspace(lo[0],hi[0],16),np.linspace(lo[1],hi[1],12))
 samples=np.c_[gx.ravel(),gz.ravel()]
 nearest=np.argmin(np.sum(((samples[:,None]-xz[None])/(hi-lo))**2,axis=2),axis=1)
 surface_ids=np.unique(np.r_[valid_ids[nearest],ids])
 surface_proj=Projection(tr['world'][10:191:5][:,surface_ids].reshape(-1,3),(589,498,137,112),az=-52,el=24)
 # One magnified texture view; identification still uses both stereo cameras.
 ...
 surface_triangles=Delaunay(tr['world'][10,surface_ids][:,[0,2]]).simplices
 # Known initial specimen geometry provides a fixed reference, not a recovered
 # back surface. The measured front surface alone is animated and highlighted.
 known=json.loads((ROOT/'out/press_separated_20260913/monotonic320/inputs_A/known.json').read_text())
 center=np.array(known['center']);size=np.array(known['size']);lo=center-size/2;hi=center+size/2
 reference_block=np.array([[x,y,z] for x in [lo[0],hi[0]] for y in [lo[1],hi[1]] for z in [lo[2],hi[2]]])
 bound=np.vstack([reference_block,tr['world'][10:191:5][:,surface_ids].reshape(-1,3)])
 surface_proj=Projection(bound,(573,490,153,122),az=-58,el=23)
 surface_origin=surface_proj.origin;surface_scale=surface_proj.scale


 reference,_,_=lerp_frame(hp['pos'],hp['times'],.05)
 displacement=np.linalg.norm(hp['pos'][hsel]-reference,axis=2)
 disp_max_m=float(np.ceil(displacement.max()/.005)*.005)
 return dict(reference_block=reference_block,texture_crop=texture_crop,surface_triangles=surface_triangles,surface_origin=surface_origin,surface_scale=surface_scale,stereo_crops=stereo_crops,surface_ids=surface_ids,surface_proj=surface_proj,hp=hp,display_pos=display_pos,kin=kin,tr=tr,force=force,camera=camera,camera_right=camera_right,ids=ids,hsel=hsel,hproj=hproj,sproj=sproj,crop=crop,rgb=rgb,reference=reference,disp_max_m=disp_max_m)

def surface_view(D,t):
 # Fixed camera and fixed reference geometry at every frame.
 return D['surface_proj']

def draw_reference_block(c,D,proj):
 corners=proj(D['reference_block'])
 # Extremely light contextual faces; the blue front mesh is the observed data.
 c.poly(corners[[1,5,7,3]],'#edf1f4')
 c.poly(corners[[4,6,7,5]],'#e6edf1')
 hidden={(0,2),(2,3),(2,6)}
 for a in range(8):
  for bit in [1,2,4]:
   b=a^bit
   ...
   edge=(a,b)
   if edge in hidden:
    start,end=corners[[a,b]];length=np.linalg.norm(end-start)
    for u in np.arange(0,length,6):
     c.line([start+(end-start)*u/length,start+(end-start)*min(u+3,length)/length],'#c7d2da',.8)
   else:c.line(corners[[a,b]],'#9fb1be',1.0)
 c.text(650,618,'Fixed reference outline',12,MUTED,anchor='mt')
...
def draw_method01(t):
 ...
 proj=surface_view(D,t);surface=tr['world'][sf,D['surface_ids']];cloud=proj(surface)
 selected=proj(tr['world'][sf,D['ids']])
 draw_reference_block(c,D,proj)
 # A faint triangulated display mesh connects only observed surface points.
 # It is not a new estimated volume or an unseen rear surface.
 dep=surface@proj.view;alpha=(dep-dep.min())/max(np.ptp(dep),1e-12)
 triangles=D['surface_triangles'];order=np.argsort(dep[triangles].mean(1))
 edges=set()
 for idx in order:
  tri=triangles[idx];shade=float(alpha[tri].mean())
  color=tuple(np.rint(np.array([204,228,239])*(1-shade)+np.array([154,197,220])*shade).astype(int))
  c.poly(cloud[tri],color)
  for j in range(3):edges.add(tuple(sorted((int(tri[j]),int(tri[(j+1)%3])))))
 for a,b in edges:c.line(cloud[[a,b]],'#85adc3',.5)
 for idx in np.argsort(dep):
  color=tuple(np.rint(np.array([110,156,182])*(1-alpha[idx])+np.array([45,102,139])*alpha[idx]).astype(int))
  c.dot(cloud[idx],1.15,color)
 for trail,dest,col in zip(trails,selected,colors):
  color=tuple(int(col[j:j+2],16) for j in [1,3,5])
  muted=tuple(np.rint(np.array(color)*.74+np.array([244,246,247])*.26).astype(int))
  c.line([trail[-1],dest],muted,1.5)
 ...
 for fid,q,col in zip(D['ids'],selected,colors):
  ...
  c.dot(q,4.1,col,INK);c.dot(q,1.2,'white')
