...
def data():
 ...
 X=kin['X'];sproj=Projection(X[::10].reshape(-1,3),(768,490,160,133),az=-52,el=24)
...
INTERIOR_NODES=[(2,3,3),(2,7,7),(5,3,7),(5,7,3),(8,3,3),(8,7,7)]
def draw_stereo_interior(c,D,frame):
 lattice=D['kin']['X'][frame].reshape(11,11,11,3);sp=D['sproj']
 def boundary(grid):return np.vstack([grid[0],grid[1:,-1],grid[-1,-2::-1],grid[-2:0:-1,0]])
 # Pale faces give the entire inferred volume context without hiding trajectories.
 for grid,fill,edge in [(lattice[:,10,:],'#edf3f6','#cadce5'),
                         (lattice[:,:,10],'#dcecf3','#a6c7d7'),
                         (lattice[10,:,:],'#d2e6f0','#91b9ce'),
                         (lattice[:,0,:],'#e4eff5','#92b8cc')]:
  rim=sp(boundary(grid));c.poly(rim,fill);c.line(np.vstack([rim,rim[0]]),edge,1)
 # The surface remains visually light; no dense wireframe obscures the interior.
 c.line(sp(lattice[0,0,:]),'#faffff',1.8)
 for node in INTERIOR_NODES:
  fid=np.ravel_multi_index(node,(11,11,11))
  path=sp(D['kin']['X'][10:frame+1,fid])
  if len(path)>1:
   c.line(path,'#ffffff',4.4);c.line(path,'#317ba4',2.35)
  c.dot(path[0],2.5,'#e4eff5','#6199b7')
  c.dot(path[-1],3.3,'#185a80','white')
 # These colored points retain the observed stereo coordinates and identities.
 for fid,col in zip(D['ids'],['#ffb321','#00d8ed','#ff65d4']):
  q=sp(D['tr']['world'][frame,fid][None])[0]
  c.dot(q,3.8,col,INK);c.dot(q,1,'white')
