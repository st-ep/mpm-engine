...
def protocol():
    ...
    return dict(E_true={'A': 80000., 'B': 240000.}, E_identified=estimates,
        ...
        nu=.45, density=1000., domain=.24, grid=160, dt=.0000125, coupling_dt=.0005,
        ...
        grip_z=.098, backstroke=.038, stroke_travel=.065, settle=.15,
        lift_delay=.08, lift_duration=.30, lift_height=.05,
        end=2.5, video_dt=.01, sdf_cell=.0005, contact_band=.00015,
        initialization=dict(damping_rate_s=100.,min_time=.30,max_time=1.,speed_tolerance=2e-5,drift_tolerance=.0000003),
        target=[.298,.15], target_radius=.025,
        bounds=[[.3,.95],[-18.,24.]], initial=[.65,12.], maxfev=30,
...
def pose(t, controls, p):
    duration, angle=controls; a=np.deg2rad(angle); c,s=np.cos(a),np.sin(a)
    R=np.array([[c,-s,0.],[s,c,0.],[0.,0.,1.]])
    travel=-p['backstroke']+p['stroke_travel']*smooth((t-p['settle'])/duration)
    z=p['grip_z']+p['lift_height']*smooth((t-p['settle']-duration-p['lift_delay'])/p['lift_duration'])
    center=np.r_[np.asarray(p['ball_initial'])[:2]+travel*R[:2,0],z]
    return center,R
...
def simulate(p, E, controls, device='cuda:0', out=None, end=None, motion=None, record_frames=True):
    ...
    result=dict(E_pa=float(E),controls=list(map(float,controls)),final_ball_m=ball.x.tolist(),error_mm=1000*error,
        ...
        particle_count=len(ref),initialization_time_s=relax_time,initialization_max_speed_m_s=speed,initialization_max_drift_m=drift,wall_s=time.monotonic()-start)
    if out is not None:
...
if __name__=='__main__':
    ...
    ap.add_argument('--device',default='cuda:0');ap.add_argument('--duration',type=float,default=.36);ap.add_argument('--angle',type=float,default=12.)
