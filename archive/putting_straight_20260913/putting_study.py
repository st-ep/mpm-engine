...
def prepare(root,source=None):
    ...
    if source is None:
        p=protocol();p.update(grid=240,dt=.0000125*160/240,coupling_dt=.000125,
            target=[.718,.23],target_radius=.035,end=1.2,video_dt=.02,
            bounds=[[.29,.55],[0.,24.]],initial=[.39,11.31],maxfev=28,early_stop_error_mm=2.,
            table_bounds=[-.03,1.18,-.18,.36,-.005,.02],
            development='The short 184 mm putt showed little A/B contrast. The 612 mm target was selected during task development, before optimizing either model. Initial searches were superseded after correcting initial gravity oscillations and resolution-dependent sampled volume. All pilots and partial searches remain in the parent directory.')
    else:
...
def execute(root,m,k,device,grid=None,stage='plan',prefix='execution',coupling=None,video=True):
    ...
    p['grid']=grid or pl['planning_grid'];p['dt']=.0000125*160/p['grid'];p['end']=4. if video else 1.2;p['video_dt']=.01
    ...
    with (root/'franka/compile.lock').open('w') as lock:
        ...
        if not motion_path.exists():compile_motion(root,label,dict(p,end=4.),u)
