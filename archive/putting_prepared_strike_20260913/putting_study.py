...
def prepare(root,source=None):
    ...
    if source is None:
        p=protocol();p.update(development='Direct strike from a statically equilibrated 18 degree ready pose. No dynamic backswing or hidden reset during the strike. Same identified material estimates, reference club, ball, floor and 612 mm target. A 160 mm grasp arc, forward swing to -22 degrees, upward follow-through from halfway through the swing over 0.18 s. Continuous-backswing alternatives are retained in the parent directory; B exhibited a 59 mm true-model miss in that variant.')
    else:
...
def plan(root,k,device,stage='plan',grid=None,maxfev=None,initial_override=None):
    ...
    def objective(u):
        for previous in history:
            if np.max(np.abs(np.asarray(previous['controls'])-u))<1e-13:return previous['score_mm']
    ...
    coarse=[]
    if stage=='plan' and initial_override is None:
        for d in p['coarse_durations']:
            objective([d,p['initial'][1]]);coarse.append([d,p['initial'][1]])
        initial=np.array(min(history,key=lambda h:h['score_mm'])['controls'])
        simplex=np.array([initial,initial+[.005,0.],initial+[0.,.8]])
    opt=minimize(objective,initial,method='Nelder-Mead',bounds=p['bounds'],callback=callback,
        options=dict(maxfev=budget-len(history),initial_simplex=simplex,xatol=.0001,fatol=.3))
    ...
    result=dict(material=k,E_pa=p['E_identified'][k],controls=best['controls'],predicted_error_mm=best['score_mm'],
        ...
        initial=list(initial),coarse_probes=coarse,initial_simplex=simplex.tolist(),objective=p['objective'],optimizer='Bounded Nelder-Mead; best evaluated candidate, finite budget, no global-optimality claim.')
