...
def summarize(root,prefix='execution'):
    ...
    lines+=['','**Control.** Optimize two continuous parameters: stroke duration and aim angle. The club starts from a statically equilibrated 18° ready pose and makes one smooth forward swing to −22°. The grasp follows a 160 mm-radius arc; the undeformed head center follows a 227.5 mm-radius arc. Prescribed upward follow-through begins at 50% of the forward duration and takes 0.18 s. Five common duration probes initialize bounded Nelder–Mead optimization in each identified model. The complete Franka command is frozen and reused on both materials. There is no force, shape, or ball-position feedback.','',
    ...
    lines+=['','**Identification inputs.** These are the existing textured-stereo bending estimates: A = 80.61 kPa and B = 248.11 kPa, against true stiffnesses of 80 and 240 kPa. They were not fitted to putting outcomes. The same homogeneous fixed-corotated material model is used in the new club geometry. The original bending reconstruction assumptions still apply.','',
        '**Numerical checks.**','',
        'The displayed executions use a 1.0 mm MPM grid and 125 μs coupling updates. The fine check uses a 0.75 mm grid; the contact-update check uses 62.5 μs updates on the 1.0 mm grid. The combined check applies both refinements to each matched plan, because the main B outcome is close to the target boundary. The frozen commands are identical in every check. Integer substepping gives MPM steps of 8.33 μs in the main run, 6.25 μs on the finer grid, and 7.81 μs in the half-contact-interval check. The combined check uses 6.25 μs MPM steps.','',
    ...
    if all(f'combined_{m}_plan_{m}' in report['numerical_checks'] for m in 'AB'):
        a,b=[report['numerical_checks'][f'combined_{m}_plan_{m}'] for m in 'AB']
        lines+=['',f"With both refinements together, matched errors are {a['error_mm']:.1f} mm (A) and {b['error_mm']:.1f} mm (B); {'both remain inside the target' if a['success'] and b['success'] else 'the matched-success outcome does not hold for both materials'}. This check also uses the original frozen commands."]
    ...
    lines+=['','The inclined ready state is prepared by numerical dynamic relaxation before time zero; no preparatory motion is implied by the video. That damping is disabled before the stroke, and elastic deformation is retained. Every resolution uses the same integrated club volume. The rigid-ball model passes independent rolling-distance and sliding-to-rolling checks.','',
        ...
        '**Development record.** The previous paused swing is archived at `out/putting_swing_20260913`. A sharper continuous backswing was tested first, but its B plan missed by 59 mm on the true material. The final study instead starts from a statically equilibrated ready pose and uses one forward arc with upward follow-through. Club geometry, material estimates, ball mechanics and the 612 mm target remain the same; the tabletop is extended to contain overshoots. Pilot alternatives and identified-model reachability pilots are retained in the parent directory. All planning evaluations are retained. No swapped-case error enters the planning objective.','',
        '**Videos.** [Synchronized strike and outcome](media/paper_putting_demo.mp4), [Stroke sequence](media/swing_sequence.png), [Comparison](media/putting_comparison.mp4), [contact close-up](media/putting_contact_closeup.mp4), [full Franka view](media/franka_putting.mp4). The main video and contact view run at real time. An additional [2× slow-motion contact view](media/putting_contact_closeup_2x.mp4) is explicitly labeled. Surfaces are interpolated from saved MPM particles; the ball stripe follows its integrated angular velocity.','',
