...
def render_case(root,m,k,prefix='execution',view='course',preview=False):
    ...
    if view=='detail':
        ids=np.flatnonzero(signals[:,0]<=1.45)[::2];W,H=800,640
    ...
    robot.forward(fk['q'][0]);geoms={};include_arm=view not in ('detail','hero')
    ...
    else:focal=np.array([.735,.18,.035]);eye=focal+[0.,-.03,1.];scale=.37
...
def compose(root,prefix='execution',view='course',slow=1):
    ...
    def canvas(i):
        ...
        d.text((55,2*h+151),f"{'2× slow motion' if slow==2 else 'Real time'}{grid_label} · {t[i]:.2f} s",font=font(20),fill=MUTED)
...
def swing_sequence(root,prefix='execution'):
    ...
    start=p['settle']
    states=[(0.,'Ready'),(start+.25*cfg['controls'][0],'Accelerating'),
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('command',choices=['render','compose','overview','sequence','demo']);ap.add_argument('--out',type=Path,required=True)
