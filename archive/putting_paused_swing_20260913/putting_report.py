...
def render_case(root,m,k,prefix='execution',view='course',preview=False):
    ...
    if view=='detail':
        ids=np.flatnonzero(signals[:,0]<=1.85);W,H=800,640
    ...
    elif view=='detail':focal=np.array([.100,.118,.090]);eye=focal+[-.22,-.30,.065];scale=.120
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('command',choices=['render','compose','overview','sequence']);ap.add_argument('--out',type=Path,required=True)
