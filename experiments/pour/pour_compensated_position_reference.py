"""Bounded original60 checks of two analytically justified arithmetic fixes.

No angle search, physical refit or validation liquid outcomes. Each GPU starts
only when free. Preserve both the original solver and the volume-only prototype.
"""
from pathlib import Path
import sys

ROOT = Path(__file__).resolve().parents[2]
WORK = ROOT / 'out/pour_physics_audit/compensated_position_20260907'
...
def prepare():
    ...
    if p.exists():
        return validate()
    base = WORK.parent / 'stable_volume_20260907'
    ...
    protocol.update(purpose=__doc__, input_sha256=frozen,
                    numerical_change='Stable log-volume/EOS plus compensated float32 particle-position accumulation',
                    implementation_gate='Analytical volume gate plus CPU/GPU translation, state and pipeline gates',
