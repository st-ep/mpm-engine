"""Explicitly revised contact calibration using only the original 60-degree replay.

The engine-default upper cap is defined in calibration_range_revision.json.
The earlier [0.05,0.20] probe and its source remain unchanged.

Requires independent review of the preceding fixed-contact timestep pair. The
revision metadata contains the original calibration endpoint; the forward
dynamics consume only the supplied contact coefficient. No angle search is run.
"""
...
def main():
    ...
    if not np.isfinite(mu) or not .05 <= mu <= .4:
        raise ValueError('Contact must remain within the revised [0.05, 0.40] interval')
    assert Path(mpm_utils.__file__).resolve().is_relative_to(source)
    candidate = source.parent
    revision_path = candidate / 'calibration_range_revision.json'
    revision = json.loads(revision_path.read_text())
    for rel, sha in revision['input_sha256'].items():
        assert digest(ROOT / rel) == sha, rel
    assert revision['new_contact_interval'] == [.05, .4]
    ...
    hashes = {**protocol['input_sha256'], **revision['input_sha256']}
    for p in [Path(__file__), Path(reference.__file__), parent, review_path, revision_path]:
        hashes[str(p.relative_to(ROOT))] = digest(p)
    for rel, sha in hashes.items():
        assert digest(ROOT / rel) == sha, rel
    work = candidate / f'recalibration_mu{mu:.6f}_n{args.grid}_dt{args.dt_scale:g}'
    ...
    write(work / 'protocol.json', dict(purpose=__doc__, input_sha256=hashes,
        ...
        grid=args.grid, dt_scale=args.dt_scale, recorded_angle_deg=60., contact_interval=[.05, .4],
        actual_kernel_module=str(Path(mpm_utils.__file__).resolve()),
        endpoint_metadata_read_from_calibration_revision=[revision['calibration_endpoint_ml']],
        measurement_used_by_forward_dynamics=False,
