"""Forward verification of target commands with the revised model frozen.

The failed 1 mL timestep screen remains disclosed. Target commands are selected
by a separate simulation-only planner. This runner cannot refit viscosity,
contact, geometry or motion, and it reads no liquid validation outcomes.
"""
...
def main():
    ...
    if not args.preflight and (args.angle is None or not ANGLE_BOUNDS[0] <= args.angle <= ANGLE_BOUNDS[1]):
        parser.error('A target angle within [45, 68] is required')
    ...
    if args.preflight:
        print(json.dumps(dict(clock_preflight_passed=True, angle_bounds_deg=ANGLE_BOUNDS,
    ...
    accepted_path = WORK / f'candidate_mu{args.contact:.6f}/target_model_protocol.json'
    assert digest(accepted_path) == accepted_path.with_suffix('.sha256').read_text().strip()
    accepted = json.loads(accepted_path.read_text())
    assert accepted['simulation_only_target_verification'] and not accepted['numerical_acceptance_passed']
    assert accepted['forecast_reported_in_chat_before_target_runs']
    assert not accepted['original_time_check_passed']
    assert not accepted['original_criteria_changed'] and not accepted['parameters_changed_after_time_check']
    assert accepted['angle_bounds_deg'][0] <= args.angle <= accepted['angle_bounds_deg'][1]
    assert abs(args.angle-round(args.angle,2)) < 1e-10
    assert time.time()-accepted['created_unix'] < accepted['wall_budget_s']
    ...
    validate()
    target_root = WORK / f'candidate_mu{args.contact:.6f}/target_cases'
    target_root.mkdir(exist_ok=True)
    destination = target_root / f'angle_{args.angle:.2f}'
    with (target_root/'.allocation.lock').open('a') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX)
        assert len(list(target_root.glob('angle_*'))) < accepted['maximum_new_target_runs']
    ...
    write_new(protocol_path, dict(purpose=__doc__, input_sha256=hashes,
        ...
        planned_angle_deg=args.angle, angle_bounds_deg=ANGLE_BOUNDS,
        ...
        fluid_validation_outcomes_used=[], maximum_new_target_runs=accepted['maximum_new_target_runs'],
        actual_kernel_module=str(Path(mpm_utils.__file__).resolve()),
        real_error_forecast_saved=False, bulk_search_queued=False,
        experimental_target_verification=True, numerical_acceptance_passed=False,
        original_time_check_passed=False, original_time_difference_ml=accepted['original_time_difference_ml']))
    ...
    write_new(destination / 'completion.json', dict(receiver_ml=result['receiver_ml'],
        ...
        experimental_target_verification=True, numerical_acceptance_passed=False, original_time_check_passed=False))
