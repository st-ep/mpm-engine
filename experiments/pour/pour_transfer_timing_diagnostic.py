"""Two diagnostic MPM probes after a marginal timestep-screen failure.

No numerical acceptance or bulk release: the failed timestep check is retained.
Angles were selected from the previous MPM table before the new calibration.
Robot timing comes only from joint/action histories. Liquid validation outcomes
are neither loaded nor used to select commands.
"""
...
def main():
    ...
    accepted_path = WORK / f'candidate_mu{args.contact:.6f}/diagnostic_probe_decision.json'
    accepted = json.loads(accepted_path.read_text())
    assert accepted['diagnostic_probes_only'] and not accepted['numerical_acceptance_passed']
    ...
    assert args.angle in accepted['allowed_angles_deg']
    ...
    validate()
    destination = WORK / f'candidate_mu{args.contact:.6f}/diagnostic_timing_probe_{args.angle:.2f}'
    ...
    write_new(protocol_path, dict(purpose=__doc__, input_sha256=hashes,
        ...
        diagnostic_only=True, numerical_acceptance_passed=False,
    ...
    write_new(destination / 'completion.json', dict(receiver_ml=result['receiver_ml'],
        ...
        diagnostic_only=True, numerical_acceptance_passed=False, original_time_check_passed=False))
