"""Separate reference runner for the revised numerical-contact calibration.

The source-contact cap is explicitly revised to the engine default 0.4.
Original reference runner and its completed [0,0.2] experiments are preserved.
See the candidate calibration_range_revision.json for the basis and inputs.
...
def run(args):
    ...
    if not np.isfinite(source_friction) or not 0. <= source_friction <= .4:
        raise ValueError("Revised numerical-contact exploration is bounded to coefficients [0, 0.4]")
