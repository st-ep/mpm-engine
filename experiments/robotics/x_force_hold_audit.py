"""Audit frozen force-and-hold executions and plot commanded/measured forces."""
...
from experiments.robotics import x_force_hold_damped_control as control
...
def audit(folder):
    ...
    for name in "AB":
        frozen = json.loads((folder / f"plans/{name}/selection_complete.json").read_text())
        assert frozen["selected_sha256"] == planning.study.digest(folder / f"plans/{name}/selected.json")
        ...
        for material in 'AB':
            for planned in 'AB':
                ...
                for i in range(4):
                    ...
                    p = next(p for p in phases if p['name'] == f'{i}:force_action')
                    times = d['time'][indices] - p['start_s'] - .002
                    modes = d['control_mode'][indices]
                    targets = np.full(len(indices), d['peaks_n'][i])
                    targets[modes == 0] = [control.ramp(t, d['peaks_n'][i]) for t in times[modes == 0]]
                    unload = np.flatnonzero(modes == 2)
                    targets[unload] *= .5 * (1 + np.cos(np.pi * ((np.arange(len(unload)) + .5) * .004 / control.CONTROL['fall_s']).clip(0, 1)))
                    np.testing.assert_allclose(targets, d['target_force'][indices], atol=1e-9)
                    hold_count = np.count_nonzero(modes == 1)
                    # Stored dwell counter counts completed servo ticks; an interrupted guard tick is recorded in the force history but not the counter.
                    completed_hold_count = hold_count - int(record['stops'][i] == 'travel_guard' and modes[-1] == 1)
                    np.testing.assert_allclose(completed_hold_count * .004, d[f'stage_{i}_actual_dwell_s'])
                    attained = bool(d[f'stage_{i}_attained'])
                    if attained:
                        preceding = indices[modes == 0][-3:]
                        assert np.all(abs(filtered[preceding] - d['peaks_n'][i]) <= .10 * d['peaks_n'][i] + 1e-10)
                        assert times[modes == 0][-1] + .002 >= .2 - 1e-9
                    if record['stops'][i] == 'dwell_complete':
                        assert attained and hold_count == round(d['durations_s'][i] / .004)
                        assert len(unload) == round(control.CONTROL['fall_s'] / .004)
                    elif record['stops'][i] == 'travel_guard':
                        assert gaps[indices[-1]] <= 12. + 1e-7
                        assert np.all(gaps[indices[:-1]] > 12. - 1e-8)
                    else:
                        assert record['stops'][i] == 'attainment_timeout'
                        assert not attained and hold_count == 0
                        assert len(indices) == round(control.CONTROL['attain_timeout_s'] / .004)
                    ...
                    for j, actual_velocity in zip(indices, v[:, 0], strict=True):
                        ...
                        trial = integral + control.CONTROL['ki_m_s2_n'] * gain_scale * error * .004
                        raw = control.CONTROL['kp_m_s_n'] * gain_scale * error + trial
                    ...
                    pulse_audits.append(dict(pinch=i + 1,
                        commanded_peak_n=float(d['peaks_n'][i]), dwell_s=float(d['durations_s'][i]),
                        force_attained=attained, actual_dwell_s=hold_count * .004,
                        hold_relative_rmse=record['diagnostics']['pulses'][i]['relative_hold_rmse'],
...
def diagnostics(folder):
    ...
    for row, (material, planned) in enumerate((a, b) for a in 'AB' for b in 'AB'):
        ...
        for i, ax in enumerate(axes[row]):
            mask = d['pulse_id'] == i
            start = next(p['start_s'] for p in phases if p['name'] == f'{i}:force_action')
            t = d['time'][mask] - start
