"""Active force-and-hold shaping figure, retaining the compact four-panel layout.

Identification/held-out validation are copied from the frozen common probe;
shaping images and scores come only from the force-and-hold true executions.
"""
...
def panel_c(ax, folder, images, xml):
    ...
    for stage, left, direction in [(2, 0, "y"), (3, width + .08, "x")]:
        ...
        ax.text(cx, .31, f"{data['peaks_n'][stage]:.2f} N · {data['durations_s'][stage]:.2f} s hold",
...
def report(folder, output, dest=None):
    ...
    for name in "AB":
        ...
        assert frozen["selected_sha256"] == study.digest(folder / f"plans/{name}/selected.json")
    ...
    for row, name in enumerate("AB"):
        ...
        for col, planned in enumerate(row_models[name]):
            ...
            if planned is not None:
                ...
                marker = ("†" if guarded[key] else "") + ("‡" if "attainment_timeout" in records[key]["stops"] else "")
    ...
    for name in ["x_force_hold_report.py", "x_common_report.py", "x_work_report.py", "x_work_probe_panel.py", "x_force_report.py",
    ...
    save_json(output / "figure_provenance.json", dict(study_directory=str(folder.resolve()),
        ...
        panel_c="Matched-A force-and-hold execution, final two pinches; labels are commanded peak mean force per finger and dwell after force attainment",
        ...
        scope="Simulation with supplied x, v and elastic F, and ideal per-finger force sensing during shaping; stock Franka Hand force-and-hold operation is not established"))
    if dest is not None:
        for suffix in [".png", ".pdf"]:
