"""Surface comparisons and force/work diagnostics for work-limited shaping."""
...
from experiments.robotics.hex_shaping_work import OUT, ROOT
...
def paper_preview(folder, images):
    ...
    source=Path(imports["models.json"]["source"]).parent
    ...
    for row,name in enumerate("AB"):
        ...
        for col,method in enumerate(["target","nominal","identified"]):
            ...
            if method!="target":
                ...
                ax.text(COLUMNS[col],ROWS[row]-.022,f"{record['error_1s_mm']:.2f} mm",ha="center",va="top",fontsize=7.2,zorder=20)
    ax.text(.5,.025,"1 s after release. Nominal A reaches the stroke limit.",ha="center",va="bottom",fontsize=6.8,color="#46515a")
    stem=folder/"identification_plastic_shaping_work"
    ...
    for row,name in enumerate("AB"):
        ...
        for col,method in enumerate(["target","nominal","identified"]):
            ...
            if method!="target":
                ...
                err=chamfer_mm(data["x_after_1s"],target["x"])
                ax.text(.5,-.015,f"{err:.3f} mm",ha="center",va="top",transform=ax.transAxes,fontsize=10)
                metrics[name][method]=dict(error_mm=err,slice_iou=section_iou(data,target,"x_after_1s"),
                    corner_to_face_ratio=[corner_to_face_ratio(xy) for xy in slices(data,target)])
    fig.text(.5,.02,"Work-limited squeezes. Same target and nominal model. Shown 1 s after release.",ha="center",fontsize=9)
...
def main():
    ...
    args=parser.parse_args();report(args.out.resolve())
