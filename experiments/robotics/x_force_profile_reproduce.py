"""Replay frozen force-profile plans in fresh true-material simulations.
...
def replay(source,folder,render,devices,prepare_only=False):
    ...
    for rel,sha in manifest.items():assert study.digest(source/rel)==sha,rel
    folder.mkdir(parents=True)
    for name in ['inputs','validation','source_snapshot']:
        shutil.copytree(source/name,folder/name)
    if (source/'gain_history').exists():
        shutil.copytree(source/'gain_history',folder/'gain_history')
    ...
    files=['protocol.json','source_sha256.json','input_sha256.json','robot_input_sha256.json',
           'gain_protocol.json','gain_source.py','gain_selected.json','execution_plan.json','profiles_A.npz','profiles_B.npz','reference_A.json','reference_B.json','target.vtp','target.json','target_outline_m.npy',
    ...
    save_json(folder/'replay_provenance.json',dict(source_archive=str(source.resolve()),
        source_manifest_sha256=study.digest(source/'checksums.json'),
    ...
    for material in 'AB':
        assert frozen['plans'][material]['profiles_sha256']==study.digest(folder/f'profiles_{material}.npz')
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        futures=[pool.submit(run,'x_force_profile_gains','evaluate',m,d)
                 for m,d in zip('AB',devices,strict=True)]
        for future in futures:future.result()
    run('x_force_profile_audit','audit')
    run('x_force_profile_audit','diagnostics')
    run('x_force_profile_report',is_render=True)
    comparisons=[]
...
if __name__=='__main__':
    ...
    p.add_argument('--source',type=Path,default=study.OUT)
