"""Plan and validate force-established dwell actions, using identified models only.

The pilot and its controller remain frozen in x_force_hold_damped_study. This stage
first finds dwell durations that approximate each identified model's existing
...
PLAN_PROTOCOL = dict(
    initial_force_factor='Unit factor of the peak force predicted by each identified-model position plan; no true data',
    duration_initialization='At minimum dwell, reduce force by 15% up to three times if already more than 0.5 mm below the model reference opening; then endpoint trials and seven bisections, retaining closest feasible prefix',
    reference='The earlier identified-model position plan, not true-material outcomes',
    force_bounds_n=[.05, 14.], duration_bounds_s=[.08, 3.],
    search='Bounded Nelder-Mead over four forces and four durations, normalized by the initialized values',
    max_evaluations=24, simplex_fraction=.05,
    objective='Symmetric area-weighted 3D surface error against the unchanged X, 1 s after full withdrawal',
    feasibility='Every dwell completes; raw mean-finger force RMSE <= 15% of command during each established dwell; height <=45.5 mm',
...
def initialize(folder, material, device):
    ...
    initial = json.loads((folder / f'inputs/initialization_{material}.json').read_text())
    peaks = np.minimum(np.array(initial['reference_peak_n']), 14.)
    durations, all_rows, decisions = [], [], []
    for i in range(4):
        goal = initial['gaps_mm'][i]
        candidates = []
        for adjust in range(4):
            short, gap = trial(folder, material, peaks[:i + 1], durations + [.08], device, f'force_initialize_{i + 1}_{adjust + 1}')
            all_rows.append(short)
            save_json(dest / 'initialization_evaluations.json', all_rows)
            if gap >= goal - .5 or adjust == 3:
                break
            peaks[i] *= .85
        low, high = .08, 3.
        for k in range(9):
    ...
    result = dict(peaks_n=peaks.tolist(), durations_s=durations, decisions=decisions,
                  ...
                  source_force_factor=1., source_reference=str(folder / f'inputs/initialization_{material}.json'))
...
def plan(folder, material, device):
    ...
    def objective(relative):
        ...
        try:
            ...
            penalty = 100 * sum(s != 'dwell_complete' for s in row['stops'])
            penalty += 100 * sum(max(0., (p['relative_hold_rmse'] or 1.) - .15) for p in row['diagnostics']['pulses'])
    ...
    assert candidates, 'No feasible complete force-and-hold plan'
    ...
    save_json(dest / 'selected.json', dict(**best, model=initial['model'],
        ...
        prior_pilot_evaluations=0, prior_position_evaluations=32,
...
def evaluate(folder, material, device, settings):
    stage(folder)
    for m in 'AB':
        frozen = json.loads((folder / f'plans/{m}/selection_complete.json').read_text())
        assert frozen['selected_sha256'] == study.digest(folder / f'plans/{m}/selected.json')
