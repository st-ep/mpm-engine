...
OUT = ROOT / 'out/x_force_hold_damped_study_20260910_v2'
...
PROTOCOL = dict(scene=core.CONFIG, control=control.CONTROL,
    ...
    pilot=dict(force_factors=[.9, 1., 1.1], durations_s=[.12, .4, .8],
    ...
    feedback='Both PI gains multiplied by 1.25/max(1.25, commanded force in N), independently of true material',
    planning='Eight controls: four mean compressive per-finger force amplitudes and four dwell durations after force attainment',
    pulse='After common 0.05 N contact detection: 0.2 s raised-cosine rise; attain within 10% for three samples within 3 s; dwell at constant force for planned time; 0.12 s raised-cosine unload',
    ...
    feasibility='All four dwells completed; per-pinch raw mean-finger force RMS error during established dwell <= 15% of command; maximum sampled height <= 45.5 mm',
...
def prepare(folder):
    ...
    source_paths += ['experiments/robotics/x_force_hold_damped_control.py', 'experiments/robotics/x_force_hold_damped_study.py']
    hashes = {}
...
def rollout(folder, dest, law, peaks, durations, device, grid=64, dt=.0001):
    ...
    feasible = all(s == 'dwell_complete' for s in stops) and diag['max_recorded_height_mm'] <= 45.5 and all(p['relative_hold_rmse'] is not None and p['relative_hold_rmse'] <= .15 for p in diag['pulses'])
