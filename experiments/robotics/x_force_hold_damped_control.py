"""Force-established dwell actions with normalized force feedback for cylindrical shaping; separate from all frozen studies.
...
CONTROL = dict(LEGACY_CONTROL, min_gap_m=.012, rise_s=.2, fall_s=.12,
               ...
               attain_tolerance=.10, attain_samples=3, attain_timeout_s=3., gain_reference_n=1.25)
CONTROL.pop("hold_s")
...
def execute(law, peaks, durations, grid=64, dt=0.00005, device="cuda:0"):
    ...
    assert durations.shape == peaks.shape and np.all((durations >= .08) & (durations <= 3.))
    ...
    for i, (peak, angle) in enumerate(zip(peaks, angles, strict=True)):
        ...
        for j in range(round((c["attain_timeout_s"] + duration + c["fall_s"]) / tick) + 2):
            ...
            raw = c["kp_m_s_n"] * gain_scale * error + trial_integral
