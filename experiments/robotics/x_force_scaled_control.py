"""Force-command pinches with amplitude-scaled PI gains.

The common gain factor is 1.25 N / max(1.25 N, commanded peak), so the
qualified 1.25 N pilot and all selected A-plan commands are unchanged.
Only command amplitude, never the actual material, sets the gain factor.
"""
...
def execute(law, peaks, grid=64, dt=0.00005, device="cuda:0"):
    ...
    data = dict(initial=x, vol0=vol, floor=np.array(cfg["floor"]),
                ...
                angles_deg=np.rad2deg(angles), gain_scale=1.25 / np.maximum(1.25, peaks))
    ...
    for i, (peak, angle) in enumerate(zip(peaks, angles, strict=True)):
        gain_scale = 1.25 / max(1.25, float(peak))
        if i:
        ...
        for j in range(round(duration / tick)):
            ...
            trial_integral = integral + gain_scale * c["ki_m_s2_n"] * error * tick
            raw = gain_scale * c["kp_m_s_n"] * error + trial_integral
