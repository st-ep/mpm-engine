"""Contact-triggered force pulses for cylindrical shaping; separate from all frozen studies.

Each action specifies force amplitude and duration. Motion follows measured
...
CONTROL = dict(LEGACY_CONTROL, min_gap_m=.012, rise_fraction=.2, fall_fraction=.2,
               contact_force_n=.05, contact_samples=3, contact_timeout_s=4.)
for _name in ["rise_s", "hold_s", "fall_s"]:
    CONTROL.pop(_name)


def pulse(t, peak, duration):
    rise, fall = CONTROL["rise_fraction"] * duration, CONTROL["fall_fraction"] * duration
    if t < rise:
        return peak * .5 * (1 - np.cos(np.pi * t / rise))
    if t <= duration - fall:
        return peak
    return peak * .5 * (1 + np.cos(np.pi * np.clip((t - duration + fall) / fall, 0, 1)))
...
def execute(law, peaks, durations, grid=64, dt=0.00005, device="cuda:0"):
    ...
    assert durations.shape == peaks.shape and np.all((durations >= .2) & (durations <= 2.4))
    ...
    for i, (peak, angle) in enumerate(zip(peaks, angles, strict=True)):
        ...
        for j in range(round(duration / tick)):
            target = pulse((j + 0.5) * tick, peak, duration)
...
def diagnostics(data, phases):
    ...
    for i, peak in enumerate(data["peaks_n"]):
        ...
        duration = data["durations_s"][i]
        plateau = selected & (t > CONTROL["rise_fraction"] * duration + .04) & (t <= (1 - CONTROL["fall_fraction"]) * duration)
        if not plateau.any():
            plateau = selected
        f = data["force_per_finger"].mean(axis=1)
        result.append(dict(
            peak_command_n=float(peak), duration_command_s=float(duration), min_gap_mm=float(data["min_gaps_mm"][i]),
